...
## nacos
```shell
export customKey="nacos"
export customValue="控制台登录密码"
docker run -d -p 8848:8848 -p 9848:9848 --name base-nacos \
  --restart always \
  -e "MODE=standalone" \
...
# 使用默认帐号密码登录：nacos/nacos，部署后登录控制台（http://部署主机IP:端口/nacos）修改密码
docker run -d -p 8848:8848 -p 9848:9848 --name base-nacos \
